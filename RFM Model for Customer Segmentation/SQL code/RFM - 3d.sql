...
(WITH rfm_value
AS
(SELECT t1.cust_id, t2.recency_diff_day, t3.frequency, CAST(t4.monetary as numeric(6,2)) FROM customers as t1
LEFT JOIN
...
WHERE frequency IS NOT NULL AND recency_diff_day IS NOT NULL AND monetary IS NOT NULL) 
-- have 51359 active customers between 2014 and 2016 
-- customer segmentation
SELECT cust_id, CAST(seg_r AS text) || CAST(seg_f AS text) || CAST(seg_m AS text) AS segment
FROM 
...
CREATE TABLE train_response_rate
AS
...
ON t1.cust_id = t4.cust_id
LEFT JOIN 
(SELECT cust_id, COUNT(DISTINCT ordernum) AS valid_contact FROM 
...
-- see the performance
SELECT * FROM validate_response_rate

COPY validate_response_rate
